...
项目已采用 Monorepo 组织（`pnpm workspace + turbo`），结构如下：
...
├─ .vscode/                 # 团队编辑器配置
├─ apps/
│  └─ web/                  # Next.js 主应用（App Router）
├─ packages/                # 可复用共享包
│  ├─ database/             # Prisma 数据层
│  ├─ fsrs/                 # 间隔重复算法
│  ├─ document-parser/      # 文档解析能力
│  ├─ ui/                   # 共享 UI 组件库
│  ├─ types/                # 共享类型定义
│  ├─ utils/                # 共享工具函数
│  └─ config/               # 共享配置
...
- `apps/web`：用户可直接访问的 Web 端，承载登录、文档管理、学习会话、统计面板与 API 路由。
- 目录内部按路由、业务域、通用组件、基础能力拆分，便于并行开发。

### 6.2 共享包层（`packages/`）

- `database`：数据模型与仓储抽象。
- `fsrs`：记忆调度核心算法，供学习引擎与 worker 复用。
- `document-parser`：多格式文档解析与 OCR 接口封装。
- `ui/types/utils/config`：组件、类型、工具、工程配置的复用基建。

### 6.3 工程层（根目录）

- `.github/workflows`：CI、CD、Release、Preview 自动化流程。
- `.changeset`：版本提案与发布驱动。
- `.husky`：本地提交门禁（lint / commit message / pre-push 检查）。
- `config`、`docker`、`scripts`：部署、构建、运行的一致化入口。
